...
export default function DatabaseOperationsPage() {
  ...
  return <main className="min-h-screen bg-[#05070b] px-4 py-6 text-slate-100 sm:px-6 lg:px-8"><div className="mx-auto max-w-6xl">
    ...
    {data && <div className="space-y-5">
      ...
      {data.schema && <section className="grid gap-3 md:grid-cols-2"><div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/[0.03] p-5"><h2 className="text-sm font-bold text-emerald-200">New schema integrity</h2><p className="mt-2 text-xs leading-6 text-slate-400">{data.schema.missingExpectedTables.length === 0 ? 'All expected Operations Center tables are present.' : `${data.schema.missingExpectedTables.length} expected table(s) are missing.`}</p>{data.schema.missingExpectedTables.length > 0 && <p className="mt-3 font-mono text-[11px] text-amber-300">{data.schema.missingExpectedTables.join(', ')}</p>}</div><div className={`rounded-2xl border p-5 ${data.schema.legacyDependency ? 'border-emerald-400/15 bg-emerald-400/[0.03]' : 'border-amber-400/15 bg-amber-400/[0.03]'}`}><h2 className="text-sm font-bold">Legacy dependency</h2><p className="mt-2 text-xs leading-6 text-slate-400">{data.schema.legacyDependency ? 'No legacy tables detected.' : 'Legacy tables still exist in the database. They are not used by the new DB boundary and should be reviewed before any destructive cleanup.'}</p>{data.schema.legacyTablesDetected.length > 0 && <p className="mt-3 font-mono text-[11px] text-amber-300">{data.schema.legacyTablesDetected.join(', ')}</p>}</div></section>}
      ...
      <section className="rounded-2xl border border-cyan-400/15 bg-cyan-400/[0.03] p-5 text-xs leading-6 text-slate-400"><strong className="text-cyan-200">Integrity rule:</strong> DATABASE_URL being present is only configuration. The console initializes the new schema and marks the database healthy only after real SQL queries succeed. Legacy tables are detected but never dropped automatically.</section>
    </div>}
  </div></main>;
}
