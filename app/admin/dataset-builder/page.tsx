    ...
    <div className="mx-auto max-w-[1400px]">
      ...
      <section className="mb-5 grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
        <article className="rounded-3xl border border-white/10 bg-white/[0.025] p-5">
          ...
          <div className="mt-5 rounded-2xl border border-cyan-400/10 bg-cyan-400/[0.03] p-4 text-xs leading-5 text-slate-400"><strong className="text-slate-200">Pipeline contract:</strong> real Deriv ticks → active feature registry → configured horizon → future direction label → chronological 70/15/15 split. Flat outcomes are excluded. No synthetic fallback is permitted.</div>
        ...
        <article className="rounded-3xl border border-white/10 bg-white/[0.025] p-5">
          ...
          <div className="space-y-4 text-sm"><div><p className="text-xs uppercase tracking-wider text-slate-500">Source policy</p><p className="mt-1 text-slate-200">Deriv real ticks only</p></div><div><p className="text-xs uppercase tracking-wider text-slate-500">Active features</p><p className="mt-1 text-slate-200">{schema ? `${schema.featureCount} · ${schema.featureSchemaVersion}` : 'Loading…'}</p></div><div><p className="text-xs uppercase tracking-wider text-slate-500">Horizon registry</p><p className="mt-1 text-slate-200">{schema ? `${schema.horizons.length} configured horizons` : 'Loading…'}</p></div><div><p className="text-xs uppercase tracking-wider text-slate-500">Leakage control</p><p className="mt-1 text-emerald-300">Outcome ticks are strictly after the feature anchor</p></div></div>
        </article>
      </section>
