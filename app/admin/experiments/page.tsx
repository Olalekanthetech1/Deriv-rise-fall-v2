...
export default function TestingResearchPage() {
  ...
  const persistExperiment = async (experimentType: string, parameters: Record<string, unknown>, experimentResult: Record<string, unknown>, horizonSeconds?: number) => {
    ...
    const response = await fetch('/api/admin/experiments', {
      ...
      body: JSON.stringify({ experimentType, symbol, horizonSeconds, parameters, result: experimentResult }),
    });
    const data = await response.json();
  ...
  const runPaperShadow = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Paper/shadow prediction failed.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return <main className="min-h-screen bg-[#05070b] px-4 py-5 text-slate-100 sm:px-6 lg:px-8">
    <div className="mx-auto max-w-[1500px]">
      ...
      <section className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-cyan-400/15 bg-cyan-400/[0.04] p-4"><div className="mb-2 flex items-center gap-2 text-xs font-semibold text-cyan-300"><Target className="h-4 w-4" />EVALUATION</div><p className="text-sm text-slate-400">Backtests and historical model evaluations use the existing server-side APIs.</p></div>
        <div className="rounded-2xl border border-purple-400/15 bg-purple-400/[0.04] p-4"><div className="mb-2 flex items-center gap-2 text-xs font-semibold text-purple-300"><Beaker className="h-4 w-4" />EXPERIMENTS</div><p className="text-sm text-slate-400">Each completed run receives a persistent experiment ID.</p></div>
        <div className="rounded-2xl border border-amber-400/15 bg-amber-400/[0.04] p-4"><div className="mb-2 flex items-center gap-2 text-xs font-semibold text-amber-300"><ShieldCheck className="h-4 w-4" />PAPER / SHADOW</div><p className="text-sm text-slate-400">Prediction-only testing. No contract is purchased or executed here.</p></div>
        <div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/[0.04] p-4"><div className="mb-2 flex items-center gap-2 text-xs font-semibold text-emerald-300"><History className="h-4 w-4" />HISTORY</div><p className="text-sm text-slate-400">Results are stored in Neon when the database is available.</p></div>
      </section>

      <nav className="mb-5 grid grid-cols-2 gap-2 rounded-2xl border border-white/10 bg-white/[0.025] p-2 sm:grid-cols-4">
        {([['backtest', 'Historical Backtest', TestTube2], ['multi-horizon', 'Multi-Horizon', Clock3], ['paper-shadow', 'Paper / Shadow', Radio], ['history', 'Experiment History', History]] as const).map(([id, label, Icon]) => <button key={id} onClick={() => { setTab(id); setError(null); }} className={`flex items-center justify-center gap-2 rounded-xl px-3 py-3 text-xs font-semibold transition ${tab === id ? 'bg-cyan-400 text-slate-950' : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'}`}><Icon className="h-4 w-4" />{label}</button>)}
      </nav>
      ...
      {tab !== 'history' && <section className="grid gap-5 lg:grid-cols-[360px_1fr]">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-5">
          <div className="flex flex-col gap-2 border-b border-white/10 pb-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs uppercase tracking-wider text-slate-500">Current target</p><h2 className="text-lg font-bold">{selectedSymbolName}</h2></div>{lastExperimentId && <span className="font-mono text-xs text-emerald-300">{lastExperimentId}</span>}</div>
          {error && <div className="mt-4 flex items-start gap-3 rounded-xl border border-red-400/20 bg-red-400/[0.05] p-4 text-sm text-red-200"><XCircle className="mt-0.5 h-4 w-4 shrink-0" />{error}</div>}
          {!result && !loading && !error && <div className="flex min-h-[320px] flex-col items-center justify-center text-center"><TestTube2 className="h-10 w-10 text-slate-700" /><p className="mt-4 text-sm font-semibold text-slate-400">No experiment result loaded</p><p className="mt-1 max-w-md text-xs leading-5 text-slate-600">Run an evaluation to see only the metrics returned by the backend.</p></div>}
          {loading && <div className="flex min-h-[320px] flex-col items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-cyan-300" /><p className="mt-3 text-sm text-slate-400">Executing against live server APIs…</p></div>}
          {result && <div className="mt-5 space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Win Rate" value={result.winRate} suffix="%" /><Metric label="Trades" value={result.totalTradesExecuted ?? result.totalTrades} /><Metric label="Net PnL" value={result.totalProfit} /><Metric label="Profit Factor" value={result.profitFactor} /></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Accuracy" value={result.accuracy} suffix="%" /><Metric label="Max Drawdown" value={result.maxDrawdown} suffix="%" /><Metric label="Sharpe-like" value={result.sharpeRatio ?? result.sharpe} /><Metric label="Best Horizon" value={result.bestHorizon} suffix="s" /></div><div className="rounded-xl border border-white/10 bg-black/20 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-500"><CheckCircle2 className="h-4 w-4 text-emerald-300" />Persisted experiment</div><p className="mt-2 font-mono text-sm text-emerald-300">{lastExperimentId || 'Not available'}</p><p className="mt-1 text-xs text-slate-600">The stored record contains the experiment type, symbol, parameters and backend result.</p></div></div>}
        </div>
      </section>}
