...
import { Activity, ArrowLeft, BrainCircuit, CheckCircle2, Clock3, Filter, Gauge, RefreshCw, Search, ShieldAlert } from 'lucide-react';

type EventRow = { id: string | number; category: string; severity: string; service: string | null; eventType: string; message: string; requestId: string | null; correlationId: string | null; symbol: string | null; modelId: string | null; createdAt: string; source: string; metadata?: Record<string, unknown> | null };
...
export default function SignalForensicsPage() {
  ...
  return <main className="min-h-screen bg-[#05070b] text-slate-100"><div className="mx-auto max-w-[1600px] px-4 py-5 sm:px-6 lg:px-8">
    ...
    <section className="mb-5 grid gap-3 grid-cols-2 lg:grid-cols-5"><Metric label="Decisions" value={decisions.length} icon={Gauge} /><Metric label="Gate accepted" value={acceptedDecisions.length} icon={CheckCircle2} /><Metric label="Prediction failures" value={failedPredictions.length} icon={ShieldAlert} /><Metric label="ML events" value={modelEvents.length} icon={BrainCircuit} /><Metric label="Evidence" value={events.length ? 1 : 0} icon={Activity} /></section>

    <section className="mb-5 rounded-2xl border border-cyan-400/10 bg-cyan-400/[0.025] p-4"><div className="mb-3 flex items-center gap-2"><Gauge className="h-4 w-4 text-cyan-300" /><h2 className="text-sm font-bold">Decision Lineage</h2></div><p className="text-xs leading-5 text-slate-500">Each completed production prediction now persists its correlation ID, final RISE/FALL/WAIT decision, confidence, strategy-gate result, risk tier, market regime, anomaly score and model coverage. Use the correlation ID to connect this decision to other telemetry.</p><div className="mt-4 grid gap-3 md:grid-cols-3"><LineageStat label="Latest decision" value={decisions[0] ? stringValue(decisions[0].metadata?.finalDecision, 'UNAVAILABLE') : 'UNAVAILABLE'} /><LineageStat label="Latest confidence" value={decisions[0] ? `${numberValue(decisions[0].metadata?.confidence)?.toFixed(1) ?? '—'}%` : 'UNAVAILABLE'} /><LineageStat label="Latest gate" value={decisions[0] ? (boolValue(decisions[0].metadata?.strategyGateAccepted) ? 'ACCEPTED' : 'BLOCKED') : 'UNAVAILABLE'} /></div></section>
    ...
    <section className="mb-5 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.025]"><div className="border-b border-white/10 p-4"><h2 className="text-sm font-bold">Production Decision Ledger</h2><p className="mt-1 text-xs text-slate-600">Only persisted prediction decisions are shown. A missing field remains unavailable.</p></div>{loading && !decisions.length ? <div className="p-12 text-center text-sm text-slate-500">Loading decision evidence…</div> : !decisions.length ? <div className="p-12 text-center"><Clock3 className="mx-auto h-8 w-8 text-slate-700" /><p className="mt-3 text-sm font-semibold text-slate-400">No persisted production decisions match the filters.</p></div> : <div className="divide-y divide-white/5">{decisions.slice(0, 100).map(event => <DecisionRow key={`${event.source}-${event.id}`} event={event} />)}</div>}</section>
...
function DecisionRow({ event }: { event: EventRow }) {
  ...
  return <article className="p-4"><div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${accepted ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-300' : 'border-amber-400/20 bg-amber-400/10 text-amber-300'}`}>{accepted ? 'EXECUTABLE' : 'WAIT / BLOCKED'}</span><span className="font-mono text-[10px] text-slate-600">{event.symbol || 'symbol unavailable'}</span><span className="font-mono text-[10px] text-slate-700">{event.correlationId || 'correlation unavailable'}</span></div><h3 className="mt-2 text-sm font-semibold text-slate-200">{stringValue(metadata.finalDecision, 'UNAVAILABLE')} · {confidence === null ? '—' : `${confidence.toFixed(1)}%`} confidence</h3><p className="mt-1 text-xs text-slate-500">{event.message}</p></div><div className="grid grid-cols-2 gap-x-6 gap-y-2 text-[10px] text-slate-600 sm:grid-cols-4"><Stat label="Risk" value={stringValue(metadata.riskTier)} /><Stat label="Regime" value={stringValue(metadata.marketRegime)} /><Stat label="Anomaly" value={anomaly === null ? '—' : anomaly.toFixed(3)} /><Stat label="Models" value={available !== null && total !== null ? `${available}/${total}` : '—'} /></div></div></article>;
}
...
function LineageStat({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-600">{label}</p><p className="mt-1 text-sm font-bold text-slate-200">{value}</p></div>; }
