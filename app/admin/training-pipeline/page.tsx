...
type Dataset = { id:string; name:string; asset_symbol:string; duration_value:number; duration_unit:string; duration_seconds:number|null; horizon_ticks:number; status:string; leakage_check_passed:boolean; sample_count:number; train_count:number };
...
export default function TrainingPipelinePage() {
  ...
  async function clearHistory() {
    if (runningRuns.length || batchActive || busy) return;
    if (!window.confirm('Clear training run history only? Datasets, samples, registered models and artifacts will be preserved.')) return;
    setBusy(true); setError(null);
    try {
