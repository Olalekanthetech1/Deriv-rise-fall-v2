...
import { ArrowLeft, BrainCircuit, CheckCircle2, Database, Play, RefreshCw, ShieldCheck, Trash2, XCircle } from 'lucide-react';
...
export default function TrainingPipelinePage() {
  ...
  const canClearHistory = !loading && !training && !clearing && runningRuns.length === 0;
  ...
  useEffect(()=>{ void load(); },[]);
  useEffect(()=>{
    if (!runningRuns.length || training || clearing) return;
    const timer = setInterval(() => void load(), 15000);
    return () => clearInterval(timer);
  },[runningRuns.length,training,clearing]);
  ...
  async function clearHistory() {
    ...
    try {
      ...
      setRuns([]);
      setMessage(`Training history cleared: ${Number(data.deletedRuns || 0).toLocaleString()} runs and ${Number(data.deletedRunModels || 0).toLocaleString()} model-run records removed. Datasets and models were preserved.`);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Unable to clear training history.');
      await load();
  ...
  return <main className="min-h-screen bg-[#05070b] px-4 py-5 text-slate-100 sm:px-6 lg:px-8"><div className="mx-auto max-w-[1500px]">
    <header className="mb-6 flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/[0.03] p-5 backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between"><div><Link href="/admin" className="mb-3 inline-flex items-center gap-2 text-xs font-semibold text-slate-500 hover:text-slate-300"><ArrowLeft className="h-3.5 w-3.5"/>Back to Operations</Link><div className="flex items-center gap-3"><div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 p-3"><BrainCircuit className="h-6 w-6 text-cyan-300"/></div><div><p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-cyan-300">Agenda 6</p><h1 className="text-2xl font-black sm:text-3xl">Model Training Pipeline</h1><p className="mt-1 text-sm text-slate-500">Native eight-model training from persisted, leakage-validated Deriv datasets.</p></div></div></div><div className="flex flex-wrap items-center gap-2"><button onClick={()=>void load()} disabled={loading||training||clearing} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-semibold text-slate-200 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading?'animate-spin':''}`}/>Refresh</button><button onClick={()=>void clearHistory()} disabled={!canClearHistory} title={runningRuns.length ? 'Finish the running training job before clearing history.' : 'Clear persisted training run history'} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-400/20 bg-red-400/[0.06] px-4 py-2.5 text-sm font-semibold text-red-200 disabled:cursor-not-allowed disabled:opacity-40"><Trash2 className="h-4 w-4"/>{clearing?'Clearing…':'Clear History'}</button></div></header>
