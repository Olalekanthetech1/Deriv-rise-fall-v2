...
function aggregatePartialRuns(runs: IngestionRun[]): IngestionRun[] {
  ...
  for (const run of runs) {
    ...
    if (!existing) { groups.set(key, { ...run }); continue; }
    ...
    existing.completedAt = existing.completedAt && run.completedAt ? new Date(Math.max(new Date(existing.completedAt).getTime(), new Date(run.completedAt).getTime())).toISOString() : existing.completedAt || run.completedAt;
...
export default function MarketDataIngestionPage() {
  ...
  const getHumanReadableAssetName = (symbolCode: string | null | undefined) => {
    ...
    return symbols.find((symbol) => symbol.symbol === symbolCode)?.displayName || 'Unavailable';
  };
  ...
  async function refresh() {
    ...
    } catch (err: any) { setError(err?.message || 'Unable to load the historical ingestion page.'); }
  ...
  function toggleSymbol(symbol: string) {
    ...
    setSelectedSymbols((current) => {
      ...
      if (selectedLimit !== null && current.length >= selectedLimit) { setError(`A maximum of ${selectedLimit} assets can be selected for one ingestion batch.`); return current; }
  ...
  function selectAll() {
    setError(null);
    const selected = selectedLimit === null ? availableSymbols.map((symbol) => symbol.symbol) : availableSymbols.slice(0, selectedLimit).map((symbol) => symbol.symbol);
    setSelectedSymbols(selected);
    setMessage(selectedLimit !== null && availableSymbols.length > selectedLimit ? `Selected the first ${selectedLimit} available assets. The remaining assets remain available for another batch.` : `Selected all ${selected.length} available assets.`);
  }

  async function handleIngest() {
    if (!selectedSymbols.length) { setError('Select at least one asset.'); return; }
    setSubmitting(true); setError(null); setMessage(null);
    try {
      const response = await fetch('/api/admin/market-data', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ symbols: selectedSymbols, count: Number(count), resumeFromCheckpoint }) });
      const data = await response.json();
    ...
    } catch (err: any) { setError(err?.message || 'Historical ingestion failed.'); }
    finally { setSubmitting(false); }
  }

  return <main className="min-h-screen bg-[#05070b] px-4 py-6 text-slate-100 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl">
    <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><Link href="/admin" className="inline-flex items-center gap-2 text-xs text-cyan-300 hover:text-cyan-200"><ArrowLeft className="h-4 w-4" />Back to Admin Operations</Link><h1 className="mt-2 text-2xl font-black tracking-tight">Market Data Ingestion</h1><p className="mt-1 max-w-3xl text-sm text-slate-500">Real Deriv historical ticks only. Select multiple assets; each asset keeps its own checkpoint and ingestion result.</p></div><button onClick={refresh} disabled={loading || submitting} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-xs font-semibold hover:bg-white/10 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Refresh</button></header>

    <section className="grid gap-4 lg:grid-cols-3"><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 lg:col-span-2"><div className="mb-4 flex items-center justify-between gap-3"><div className="flex items-center gap-2 text-sm font-semibold text-cyan-300"><Radio className="h-4 w-4" />Historical backfill</div><span className="text-xs text-slate-500">{selectedSymbols.length} selected</span></div>
      <div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div className="text-xs font-semibold uppercase tracking-wider text-slate-500">Deriv assets</div><div className="flex gap-2"><button type="button" onClick={selectAll} disabled={!availableSymbols.length || submitting} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] font-semibold hover:bg-white/10 disabled:opacity-50">{selectAllLabel}</button><button type="button" onClick={() => setSelectedSymbols([])} disabled={submitting} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] font-semibold hover:bg-white/10 disabled:opacity-50">Clear</button></div></div>
        <div className="grid max-h-80 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">{availableSymbols.length === 0 ? <div className="col-span-full text-sm text-slate-500">No available assets loaded.</div> : availableSymbols.map((asset) => <label key={asset.symbol} className={`flex cursor-pointer items-center gap-3 rounded-lg border px-3 py-2.5 text-sm transition ${selectedSymbols.includes(asset.symbol) ? 'border-cyan-400/30 bg-cyan-400/5' : 'border-white/10 bg-black/10 hover:bg-white/5'}`}><input type="checkbox" checked={selectedSymbols.includes(asset.symbol)} onChange={() => toggleSymbol(asset.symbol)} disabled={submitting} className="h-4 w-4 accent-cyan-400" /><span className="min-w-0"><span className="block truncate font-semibold text-slate-200">{asset.displayName}</span></span></label>)}</div>
      </div>
      <div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block"><span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-slate-500">Ticks per asset</span><input value={count} onChange={(e) => setCount(e.target.value)} inputMode="numeric" className="w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm outline-none focus:border-cyan-400/50" /></label><div className="rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-slate-400"><span className="block text-xs font-semibold uppercase tracking-wider text-slate-500">Batch execution</span><span className="mt-1 block text-slate-200">Up to {runtime?.concurrency ?? 'configured'} assets concurrently; each checkpoint remains independent.</span></div></div>
