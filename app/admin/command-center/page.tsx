...
function StatusPill({ status }: { status: 'healthy' | 'degraded' | 'unavailable' | 'unknown' }) {
  const config = {
  ...
  return <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold tracking-wider ${config.className}`}><Icon className="h-3.5 w-3.5" />{config.label}</span>;
}
