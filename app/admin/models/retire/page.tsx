    ...
    <main className="min-h-screen bg-[#05070b] text-slate-100">
      <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-5">
          ...
          {loading ? <div className="flex justify-center py-12 text-slate-500"><RefreshCw className="h-5 w-5 animate-spin" /></div> : productionModels.length === 0 ? <div className="rounded-xl border border-dashed border-white/10 p-10 text-center text-sm text-slate-500">No active production models.</div> : <div className="space-y-3">{productionModels.map((model) => <div key={model.model_id} className="flex flex-col gap-4 rounded-xl border border-white/10 bg-black/20 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold text-slate-100">{model.model_name || 'Unknown model'}</p><p className="mt-1 font-mono text-[10px] text-slate-500">{model.model_id}</p><p className="mt-1 text-xs text-slate-400">{model.asset_display_name || model.symbol || '—'} · {model.horizon_secs ?? '—'}s · PRODUCTION</p></div><button onClick={() => void retire(model)} disabled={busy === model.model_id} className="inline-flex items-center justify-center gap-2 rounded-xl border border-amber-400/25 bg-amber-400/10 px-4 py-2 text-xs font-bold text-amber-200 hover:bg-amber-400/15 disabled:opacity-50">{busy === model.model_id ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Archive className="h-4 w-4" />}Retire</button></div>)}</div>}
        </section>
      </div>
    </main>
  );
