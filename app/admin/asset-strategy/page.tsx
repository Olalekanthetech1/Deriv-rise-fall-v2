...
export default function AssetStrategyPage() {
  ...
  const load = useCallback(async (showSpinner = false) => {
    ...
    setError(null);
    try {
      const [runResponse, registryResponse] = await Promise.all([
        fetch('/api/admin/model-training', { cache: 'no-store' }),
        fetch('/api/ml/registry', { cache: 'no-store' }),
      ]);
      if (runResponse.status === 401 || registryResponse.status === 401) {
        window.location.replace('/admin');
        return;
      }
      const [runPayload, registryPayload] = await Promise.all([
      ...
      if (!registryResponse.ok || registryPayload?.success === false) throw new Error(registryPayload?.error || `Model registry returned HTTP ${registryResponse.status}.`);
      setRuns(Array.isArray(runPayload?.runs) ? runPayload.runs : []);
      setModels(Array.isArray(registryPayload?.models) ? registryPayload.models : []);
    } catch (err) {
