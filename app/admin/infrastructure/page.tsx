...
type InfrastructureData = {
  ...
  database: { status: Status; configured: boolean; source: string };
...
export default function RuntimeInfrastructurePage() {
  ...
  return <main className="min-h-screen bg-[#05070b] px-4 py-5 text-slate-100 sm:px-6 lg:px-8"><div className="mx-auto max-w-[1500px]">
    ...
    <section className="grid gap-5 lg:grid-cols-2">
      ...
      <Panel title="Database" icon={Database} status={data?.database.status || 'unavailable'}><Row label="Connection configured" value={data ? (data.database.configured ? 'YES' : 'NO') : '—'} /><Row label="Evidence source" value={data?.database.source || '—'} /><p className="mt-4 text-xs leading-5 text-slate-500">Connection state is reported only when the live health endpoint exposes it. Configuration presence alone is never treated as a successful database connection.</p></Panel>
