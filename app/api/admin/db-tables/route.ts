...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [registry, backtests, audits, trades] = await Promise.all([
      sql`SELECT * FROM ml_model_registry ORDER BY id DESC LIMIT 50`,
      sql`SELECT * FROM ml_backtest_results ORDER BY tested_at DESC LIMIT 50`,
      sql`SELECT * FROM ml_performance_audit ORDER BY created_at DESC LIMIT 50`,
      sql`SELECT * FROM trades ORDER BY executed_at DESC LIMIT 50`,
    ]);
