...
async function worker(jobId: string) {
  if (activeWorkers.has(jobId) || activeWorkers.size >= MAX_WORKERS) return;
...
function resume(jobIds: string[]) { for (const id of [...new Set(jobIds)]) { if (activeWorkers.size >= MAX_WORKERS) break; void worker(id); } }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!jobs.length) return NextResponse.json({ success: false, error: 'None of the selected assets had a supported build scope.', results }, { status: 422, headers: noStore() });
    resume(jobs.map((job) => job.id));
    return NextResponse.json({ success: true, mode: 'MULTI_ASSET_DATASET_BUILD', selectedSymbols: symbols, autoJobIds: jobs.map((job) => job.id), jobs, results, message: buildAll ? `Dataset builds started for ${jobs.length} assets using their own supported horizon ladders.` : `Dataset build started for ${jobs.length} assets at the selected horizon.` }, { status: 202, headers: noStore() });
