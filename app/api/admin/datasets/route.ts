...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, datasets }, { headers: noStore() });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const windowTicks = Number(body?.windowTicks);
    ...
    if (!Number.isSafeInteger(horizonTicks) || horizonTicks < 1 || horizonTicks > 5000) {
      return NextResponse.json({ success: false, error: 'horizonTicks must be an integer between 1 and 5,000.' }, { status: 400, headers: noStore() });
    }
    if (!Number.isSafeInteger(windowTicks) || windowTicks !== 300) {
      return NextResponse.json({ success: false, error: 'The canonical 37-feature dataset requires a 300-tick feature window.' }, { status: 400, headers: noStore() });
    }
    ...
    const status = /insufficient|required|canonical|No non-flat/i.test(message) ? 422 : 500;
