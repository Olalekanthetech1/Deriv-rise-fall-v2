...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const payload = await healthResponse.json().catch(() => ({}));
    if (payload && typeof payload === 'object') health = payload as Record<string, unknown>;
  } catch (error) {
    healthError = error instanceof Error ? error.message : 'Health endpoint unavailable.';
  }
