...
function auth(req: NextRequest) { const cookie = req.cookies.get('admin_session_token')?.value; const header = req.headers.get('x-admin-token') || req.headers.get('authorization')?.replace(/^Bearer\s+/i, ''); return verifySessionToken(cookie) || verifySessionToken(header); }
...
function sequencePartitions(rows: any[], sequenceLength: number, schema: { featureCount: number; featureSchemaVersion: string; schemaFingerprint: string }) {
  ...
  for (const split of ['train', 'validation', 'test']) {
    ...
    for (let i = sequenceLength - 1; i < ordered.length; i += 1) {
      ...
      if (vectors.some((vector: number[] | null) => !vector || vector.length !== schema.featureCount || vector.some((value: number) => !Number.isFinite(value)))) continue;
...
export async function GET(req: NextRequest) {
  ...
  const url = getDbConnectionString(); if (!url || !(await initDbSchema())) return NextResponse.json({ error: 'Database unavailable.' }, { status: 503, headers: noStore() });
  const sql = neon(url); await ensureTrainingDurationSchema(sql); const symbol = req.nextUrl.searchParams.get('symbol')?.trim().toUpperCase();
  const rows = symbol ? await sql`SELECT id,name,asset_symbol,duration_value,duration_unit,duration_seconds,horizon_type,horizon_ticks,status,leakage_check_passed,sample_count,train_count,validation_count,test_count,feature_schema_version,metadata,created_at FROM training_datasets WHERE asset_symbol = ${symbol} ORDER BY created_at DESC LIMIT 100` : await sql`SELECT id,name,asset_symbol,duration_value,duration_unit,duration_seconds,horizon_type,horizon_ticks,status,leakage_check_passed,sample_count,train_count,validation_count,test_count,feature_schema_version,metadata,created_at FROM training_datasets ORDER BY created_at DESC LIMIT 100`;
...
export async function POST(req: NextRequest) {
  if (!auth(req)) return NextResponse.json({ error: 'Unauthorized admin access.' }, { status: 401, headers: noStore() });
  try {
    const body = await req.json().catch(() => ({})); const datasetId = token(body?.datasetId);
    ...
    const schema = await import('@/lib/ml-runtime-schema').then((m) => m.getMlRuntimeSchemaContract()); const featureCount = Number(schema.featureCount); const sequenceLength = Number(schema.sequenceLength); const partitions: Record<string, any> = {};
    for (const split of ['train','validation','test']) {
      const splitRows = samples.filter((row: any) => row.split === split); const vectors = splitRows.map((row: any) => Array.isArray(row.feature_vector) ? row.feature_vector.map(Number) : null);
      if (vectors.some((vector: number[] | null) => !vector || vector.length !== featureCount || vector.some((value: number) => !Number.isFinite(value)))) return NextResponse.json({ error: `Invalid persisted feature vector in ${split} partition.` }, { status: 422, headers: noStore() });
    ...
    const requested = Array.isArray(body?.modelTypes) ? body.modelTypes.filter((v: unknown) => typeof v === 'string') : []; const definitions = getMlModelDefinitions().filter((definition) => requested.length === 0 || requested.includes(definition.key));
    if (!definitions.length) return NextResponse.json({ error: 'No registered model types were requested.' }, { status: 400, headers: noStore() });
    const runId = crypto.randomUUID(); const assetRows = await sql`SELECT asset_class FROM market_assets WHERE symbol = ${String(dataset.asset_symbol)} LIMIT 1`; const assetClass = String(assetRows[0]?.asset_class || 'unknown'); const results: any[] = [];
    for (const definition of definitions) {
      const sequenceModel = definition.family === 'sequential'; const result = await xgboostDaemon.sendCommand('train_partitioned', { symbol: String(dataset.asset_symbol), modelType: definition.key, durationValue, durationUnit, durationSeconds, effectiveHorizonTicks, trainTabularDataset: sequenceModel ? undefined : partitions.train, validationTabularDataset: sequenceModel ? undefined : partitions.validation, trainSequenceDataset: sequenceModel ? sequence.train : undefined, validationSequenceDataset: sequenceModel ? sequence.validation : undefined, datasetId, hyperparams: definition.defaultHyperparameters });
      if (!result?.success) { results.push({ modelType: definition.key, success: false, error: result?.error || 'Native training failed' }); continue; }
      const modelId = String(result.modelId); await registerDurationModel({ modelId, modelFamily: definition.family, version: `${String(schema.featureSchemaVersion)}-${Date.now()}`, symbol: String(dataset.asset_symbol), assetClass, durationValue, durationUnit, durationSeconds, effectiveHorizonTicks, datasetId, format: sequenceModel ? 'PT_STATE' : 'PKL', status: 'candidate', featureSchemaVersion: String(schema.featureSchemaVersion), framework: sequenceModel ? 'pytorch' : definition.key, trainingRunId: runId, metrics: { ...(result.metrics || {}), engine: result.engine, samplesCount: result.samplesCount, validationSamples: result.validationSamples }, hyperparameters: definition.defaultHyperparameters });
  ...
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : 'Duration model training failed.' }, { status: 500, headers: noStore() }); }
}
