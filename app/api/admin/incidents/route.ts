...
async function syncRecentIncidents(sql: NeonQueryFunction<false, false>) {
  ...
  for (const row of rows as any[]) {
    const fingerprintResult = await sql`
      SELECT md5(concat_ws('|', ${row.category}, ${row.service ?? ''}, ${row.event_type}, ${row.message}, ${row.symbol ?? ''}, ${row.model_id ?? ''})) AS fingerprint
    `;
    const fingerprint = String((fingerprintResult as any[])[0]?.fingerprint || '');
    ...
    await sql`
      ...
      VALUES
        (${fingerprint}, ${row.severity}, 'open', ${row.event_type}, ${row.message}, ${row.service ?? null}, ${row.symbol ?? null}, ${row.model_id ?? null}, ${Number(row.id) || null}, ${JSON.stringify(row.metadata ?? {})}::jsonb, ${row.created_at}, ${row.created_at}, NOW())
