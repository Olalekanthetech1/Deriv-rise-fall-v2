...
export async function trainDatasetModels(request: TrainingRequest) {
  ...
  const sequence = sequencePartitions(samples, schema.sequenceLength, schema);
  ...
  await sql`INSERT INTO ml_training_runs (run_id,dataset_id,asset_symbol,duration_value,duration_unit,duration_seconds,horizon_ticks,status,requested_models,started_at,metadata) VALUES (${runId},${datasetId},${String(dataset.asset_symbol)},${durationValue},${durationUnit},${durationSeconds},${effectiveHorizonTicks},'running',${JSON.stringify(definitions.map((d)=>d.key))}::jsonb,NOW(),${JSON.stringify({featureSchemaVersion:schema.featureSchemaVersion,schemaFingerprint:schema.schemaFingerprint})}::jsonb)`;
  ...
  const results: any[] = []; let completed = 0; let failed = 0;
  const assetRows = await sql`SELECT asset_class FROM market_assets WHERE symbol=${String(dataset.asset_symbol)} LIMIT 1`;
  ...
  for (const definition of definitions) {
    ...
    try {
      ...
      const result = await xgboostDaemon.sendCommand('train_partitioned', { symbol:String(dataset.asset_symbol),modelType:definition.key,durationValue,durationUnit,durationSeconds,effectiveHorizonTicks,datasetId,trainTabularDataset:sequenceModel?undefined:partitions.train,validationTabularDataset:sequenceModel?undefined:partitions.validation,trainSequenceDataset:sequenceModel?sequence.train:undefined,validationSequenceDataset:sequenceModel?sequence.validation:undefined,hyperparams:definition.defaultHyperparameters });
      if (!result?.success) throw new Error(result?.error || 'Native training failed.');
      const modelId = String(result.modelId);
      await registerDurationModel({ modelId,modelFamily:definition.family,version:`${schema.featureSchemaVersion}-${runId.slice(0,8)}`,symbol:String(dataset.asset_symbol),assetClass,durationValue,durationUnit,durationSeconds,effectiveHorizonTicks,datasetId,format:sequenceModel?'PT_STATE':'PKL',status:'candidate',featureSchemaVersion:String(schema.featureSchemaVersion),framework:sequenceModel?'pytorch':definition.key,trainingRunId:runId,metrics:{...(result.metrics||{}),engine:result.engine,samplesCount:result.samplesCount,validationSamples:result.validationSamples},hyperparameters:definition.defaultHyperparameters });
      await sql`UPDATE ml_training_run_models SET status='completed',model_id=${modelId},metrics=${JSON.stringify({...(result.metrics||{}),engine:result.engine,samplesCount:result.samplesCount,validationSamples:result.validationSamples})}::jsonb,completed_at=NOW() WHERE run_id=${runId} AND model_type=${definition.key}`;
      completed += 1; results.push({modelType:definition.key,success:true,modelId,metrics:result.metrics||{},engine:result.engine});
    } catch (error) {
      ...
      failed += 1; results.push({modelType:definition.key,success:false,error:message});
    }
    const done = completed + failed;
    await updateRun(sql,runId,done===definitions.length?(failed===0?'completed':completed>0?'partial':'failed'):'running',completed,failed,done===definitions.length?new Date().toISOString():null,{progress:{completed,failed,total:definitions.length},featureSchemaVersion:schema.featureSchemaVersion,schemaFingerprint:schema.schemaFingerprint});
  }
  return {runId,status:failed===0?'completed':completed>0?'partial':'failed',completedModels:completed,failedModels:failed,totalModels:definitions.length,dataset:{id:datasetId,symbol:dataset.asset_symbol,durationValue,durationUnit,durationSeconds,effectiveHorizonTicks},results};
}

export async function listTrainingRuns(symbol?: string) {
  const url = getDbConnectionString(); if (!url || !(await initDbSchema())) throw new Error('DATABASE_UNAVAILABLE');
  const sql = neon(url); await ensureTrainingDurationSchema(sql);
  const rows = symbol ? await sql`SELECT r.*,COALESCE(json_agg(m ORDER BY m.created_at) FILTER (WHERE m.id IS NOT NULL),'[]'::json) AS models FROM ml_training_runs r LEFT JOIN ml_training_run_models m ON m.run_id=r.run_id WHERE r.asset_symbol=${symbol} GROUP BY r.run_id ORDER BY r.created_at DESC LIMIT 50` : await sql`SELECT r.*,COALESCE(json_agg(m ORDER BY m.created_at) FILTER (WHERE m.id IS NOT NULL),'[]'::json) AS models FROM ml_training_runs r LEFT JOIN ml_training_run_models m ON m.run_id=r.run_id GROUP BY r.run_id ORDER BY r.created_at DESC LIMIT 50`;
  return rows;
}
