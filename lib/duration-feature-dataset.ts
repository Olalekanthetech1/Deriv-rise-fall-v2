...
export interface DurationFeatureContext {
  symbol: string;
  duration: DurationSpec;
  assetCategory: number;
}

export interface DurationTabularFeatureDataset {
...
export interface DurationSequenceFeatureDataset {
...
function futureIndex(ticks: TickPoint[], anchorIndex: number, duration: DurationSpec): number {
  ...
  while (lo <= hi) {
    ...
    const epoch = Number(ticks[mid].timestamp ?? 0) / 1000;
...
function vectorForTicks(ticks: TickPoint[], context: DurationFeatureContext): number[] {
  return featureObjToArray(extractTickFeatures(ticks, {
    ...
    contractDurationSecs: context.duration.seconds ?? 0,
...
function validateContext(context: DurationFeatureContext): void {
  ...
  if (context.duration.unit !== 't' && (!Number.isFinite(context.duration.seconds) || Number(context.duration.seconds) <= 0)) {
    throw new Error('INVALID_DURATION_SECONDS');
  }
}

export async function buildDurationTabularFeatureDataset(
  ticks: TickPoint[],
  context: DurationFeatureContext,
): Promise<DurationTabularFeatureDataset> {
  validateContext(context);
  const schema = await getMlRuntimeSchemaContract();
  const canonicalWindow = schema.canonicalFeatureWindowTicks;
  if (ticks.length <= canonicalWindow + 1) throw new Error('INSUFFICIENT_TICKS');

  const prices = ticks.map(priceOf);
  const featureVectors: number[][] = [];
  const labels: number[] = [];
  for (let i = canonicalWindow; i < prices.length - 1; i += 1) {
    const outcomeIndex = futureIndex(ticks, i, context.duration);
    if (outcomeIndex < 0 || outcomeIndex >= ticks.length) continue;
    featureVectors.push(vectorForTicks(ticks.slice(0, i + 1), context));
    labels.push(prices[outcomeIndex] > prices[i] ? 1 : 0);
  }
...
export async function buildDurationSequenceFeatureDataset(
  ticks: TickPoint[],
  context: DurationFeatureContext,
): Promise<DurationSequenceFeatureDataset> {
  validateContext(context);
  const schema = await getMlRuntimeSchemaContract();
  const sequenceLength = schema.sequenceLength;
  if (ticks.length <= sequenceLength + 1) throw new Error('INSUFFICIENT_SEQUENCE_TICKS');

  const prices = ticks.map(priceOf);
  const featureSequences: number[][][] = [];
  const labels: number[] = [];
  for (let i = sequenceLength; i < prices.length - 1; i += 1) {
    const outcomeIndex = futureIndex(ticks, i, context.duration);
    if (outcomeIndex < 0 || outcomeIndex >= ticks.length) continue;
    const sequence: number[][] = [];
    for (let j = i - sequenceLength + 1; j <= i; j += 1) {
      sequence.push(vectorForTicks(ticks.slice(0, j + 1), context));
    }
