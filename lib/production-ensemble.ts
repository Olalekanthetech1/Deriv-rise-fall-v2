...
export interface ProductionEnsembleResult {
  ...
  evaluations: Array<{ modelKey: string; modelName: string; family: 'tabular' | 'sequential' | 'regime' | 'anomaly'; status: ModelStatus; probabilityUp: number | null; probabilityDown: number | null; signal: Signal | null; confidence: number | null; dynamicWeight: number | null; runtimeMode: string; details: string; validation: any }>;
...
const MODEL_NAMES: Record<string, string> = { xgboost: 'XGBoost', lightgbm: 'LightGBM', catboost: 'CatBoost', tcn: 'TCN', lstm: 'LSTM / GRU', transformer: 'Transformer' };

function familyForModel(key: string): 'tabular' | 'sequential' { return ['xgboost', 'lightgbm', 'catboost'].includes(key) ? 'tabular' : 'sequential'; }
function finiteProbability(value: unknown): number | null { const n = Number(value); return Number.isFinite(n) && n >= 0 && n <= 100 ? n : null; }
function validationWeight(result: any): number | null { const accuracy = Number(result?.validation?.accuracy); const logLoss = Number(result?.validation?.logLoss); if (!Number.isFinite(accuracy) || !Number.isFinite(logLoss) || logLoss < 0) return null; const weight = accuracy / (1 + logLoss); return Number.isFinite(weight) && weight > 0 ? weight : null; }
...
export async function evaluateProductionEnsemble(ticks: TickPoint[], options: { symbol?: string; durationSecs?: number; assetCategory?: number } = {}): Promise<ProductionEnsembleResult> {
  ...
  const remote = await xgboostDaemon.sendCommand('predict_ensemble', { symbol, durationSecs: Number(durationSecs), assetCategory, featureVector, featureSequence });
  ...
  const evaluations = PREDICTIVE_MODELS.map((key) => {
    ...
    return { modelKey: key, modelName: MODEL_NAMES[key], family: familyForModel(key), status: valid ? ('AVAILABLE' as const) : ('UNAVAILABLE' as const), probabilityUp: valid ? up : null, probabilityDown: valid ? down : null, signal: valid ? (up! >= down! ? ('RISE' as const) : ('FALL' as const)) : null, confidence: valid ? Math.max(up!, down!) : null, dynamicWeight, runtimeMode: valid ? 'Native Python trained model' : 'Unavailable — no synthetic fallback', details: valid ? String(result.engine || 'Native trained model') : String(result?.error || 'MODEL_UNAVAILABLE'), validation: result?.validation || null };
  });
  const available = evaluations.filter((e): e is typeof evaluations[number] & { probabilityUp: number; probabilityDown: number; dynamicWeight: number } => e.status === 'AVAILABLE' && e.probabilityUp !== null && e.probabilityDown !== null && e.dynamicWeight !== null);
  ...
  modelBreakdown.hmm = hmmAvailable ? { modelName: 'HMM Regime Model', status: 'AVAILABLE', primaryRegime: hmm.primaryRegime, regimeState: hmm.regimeState, regimeProbabilities: hmm.regimeProbabilities, details: hmm.engine || 'Native trained GaussianHMM' } : { modelName: 'HMM Regime Model', status: 'UNAVAILABLE', details: String(hmm?.error || 'MODEL_UNAVAILABLE') };
  modelBreakdown.isolation_forest = isoAvailable ? { modelName: 'Isolation Forest', status: 'AVAILABLE', anomalyScore: iso.anomalyScore, isAnomaly: Boolean(iso.isAnomaly), details: iso.engine || 'Native trained IsolationForest' } : { modelName: 'Isolation Forest', status: 'UNAVAILABLE', details: String(iso?.error || 'MODEL_UNAVAILABLE') };

  const normalizedWeights = Object.fromEntries(available.map(e => [e.modelKey, Number((e.dynamicWeight / totalWeight).toFixed(6))]));
  return {
    symbol, direction, probUp: Number(probUp.toFixed(2)), probDown: Number(probDown.toFixed(2)), confidence: Number(confidence.toFixed(2)), marketRegime, anomalyScore, features, evaluations, modelBreakdown,
    ...
    drift: { modelWeights: normalizedWeights, recentAccuracies: Object.fromEntries(available.map(e => [e.modelKey, e.validation?.accuracy ?? null])), driftStatus: 'Weights derived from persisted native-model validation metrics', topPerformingModel: available.slice().sort((a, b) => b.dynamicWeight - a.dynamicWeight)[0].modelKey },
