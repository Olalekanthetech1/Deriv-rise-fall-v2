...
export interface SignalDuration { value: number; unit: 't' | 'm' | 'h'; label: string; seconds: number; }
...
export interface SignalConsensus {
  direction: SignalDirection | 'WAIT'; confidence: number; agreement: number; totalEngines: number;
  recommendedDuration: SignalDuration; expiresAt: number; expiresInSeconds: number;
  status: 'ACTIVE' | 'EXPIRING' | 'EXPIRED' | 'WAIT'; modeRecommendations: SignalModeRecommendation[];
}
...
export function createDuration(value: number, unit: 't' | 'm' | 'h', label: string): SignalDuration { return { value, unit, label, seconds: durationToSeconds(value, unit) }; }
...
export function buildConsensus(signals: Array<{
  ...
  recommendedDurationValue: number; recommendedDurationUnit: 't' | 'm' | 'h'; recommendedDurationLabel: string;
  ...
  for (const signal of aligned) {
    ...
    if (existing) { existing.count += 1; existing.confidence += signal.confidence; }
    else durationVotes.set(key, { count: 1, confidence: signal.confidence, sample: signal });
  }
  ...
  const executable = agreement >= 0.75 && confidence >= 70;
  const expiresAt = now + duration.seconds * 1000;

  return {
    direction: executable ? winner : 'WAIT', confidence: Number(confidence.toFixed(1)), agreement: Number((agreement * 100).toFixed(1)),
    totalEngines: signals.length, recommendedDuration: duration, expiresAt, expiresInSeconds: duration.seconds,
    status: executable ? getSignalStatus(expiresAt, now) : 'WAIT', modeRecommendations: [],
  };
}
