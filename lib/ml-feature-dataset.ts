...
export interface FeatureVectorContext {
  symbol: string;
  durationSecs: number;
  assetCategory: number;
}

export interface TabularFeatureDataset {
...
export interface SequenceFeatureDataset {
...
export async function buildFeatureVector(
  ...
  if (vector.length !== schema.featureCount) {
    throw new Error(`FEATURE_VECTOR_LENGTH_MISMATCH: expected ${schema.featureCount}, got ${vector.length}`);
  }
  return vector;
}

export async function buildTabularFeatureDataset(
  ticks: TickPoint[],
  context: FeatureVectorContext,
): Promise<TabularFeatureDataset> {
  const schema = await getMlRuntimeSchemaContract();
  const canonicalWindow = schema.canonicalFeatureWindowTicks;
  const horizon = Math.max(1, Math.trunc(context.durationSecs));
  if (ticks.length <= canonicalWindow + horizon) {
    throw new Error(`INSUFFICIENT_TICKS: need more than ${canonicalWindow + horizon} ticks`);
  }
...
export async function buildSequenceFeatureDataset(
  ticks: TickPoint[],
  context: FeatureVectorContext,
): Promise<SequenceFeatureDataset> {
  const schema = await getMlRuntimeSchemaContract();
  const sequenceLength = schema.sequenceLength;
  const horizon = Math.max(1, Math.trunc(context.durationSecs));
  if (ticks.length <= sequenceLength + horizon) {
    throw new Error(`INSUFFICIENT_TICKS: need more than ${sequenceLength + horizon} ticks`);
  }
  ...
  for (let i = sequenceLength; i < prices.length - horizon; i += 1) {
    ...
    for (let j = i - sequenceLength + 1; j <= i; j += 1) {
      sequence.push(vectorForTicks(ticks.slice(0, j), context));
    }
  ...
  return {
    featureSequences,
    labels,
    sampleCount: featureSequences.length,
    featureCount: schema.featureCount,
    sequenceLength,
    schemaVersion: schema.featureSchemaVersion,
    schemaFingerprint: schema.schemaFingerprint,
...
export async function buildBacktestFeatureVectors(
  ticks: TickPoint[],
  horizon: number,
  context: FeatureVectorContext,
): Promise<number[][]> {
  ...
  for (let i = canonicalWindow; i < ticks.length - horizon; i += 1) {
    vectors.push(vectorForTicks(ticks.slice(0, i), context));
  }
  return vectors;
}
