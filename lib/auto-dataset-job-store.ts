...
export type AutoDatasetJobItem = {
  id: number;
  itemIndex: number;
  value: number;
...
async function ensureSchema(): Promise<void> {
  ...
  schemaReady = (async () => {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS ops_ml_dataset_build_jobs (
        id UUID PRIMARY KEY,
    ...
    `;
    await sql`
      CREATE INDEX IF NOT EXISTS idx_ops_ml_dataset_jobs_symbol_started
      ON ops_ml_dataset_build_jobs (symbol, started_at DESC)
    `;
    await sql`
      CREATE UNIQUE INDEX IF NOT EXISTS uq_ops_ml_dataset_jobs_running_symbol
      ON ops_ml_dataset_build_jobs (symbol)
      WHERE status = 'running'
    `;
    await sql`
      CREATE TABLE IF NOT EXISTS ops_ml_dataset_build_job_items (
        id BIGSERIAL PRIMARY KEY,
        job_id UUID NOT NULL REFERENCES ops_ml_dataset_build_jobs(id) ON DELETE CASCADE,
        item_index INTEGER NOT NULL,
        duration_value INTEGER NOT NULL,
        duration_unit VARCHAR(1) NOT NULL,
        duration_range_id VARCHAR(160) NOT NULL,
    ...
    `;
    await sql`
      CREATE INDEX IF NOT EXISTS idx_ops_ml_dataset_job_items_claim
      ON ops_ml_dataset_build_job_items (job_id, status, claimed_at, item_index)
    `;
  })();
  try {
    await schemaReady;
...
export async function createAutoDatasetJob(
  symbol: string,
  durations: Array<{ value: number; unit: DerivDurationUnit; rangeId: string }>,
): Promise<AutoDatasetJob> {
  await ensureSchema();
  const sql = getDbOrThrow();

  const running = await sql`
    SELECT * FROM ops_ml_dataset_build_jobs
  ...
  try {
    await sql`
      ...
      VALUES (${id}, ${symbol}, 'running', ${durations.length})
    `;
  } catch (error) {
    const retry = await sql`
      ...
      LIMIT 1
    `;
    if (retry.length) return mapJob(retry[0]);
    throw error;
  }

  if (durations.length) {
    const jobIds = durations.map(() => id);
    ...
    await sql`
      INSERT INTO ops_ml_dataset_build_job_items (job_id, item_index, duration_value, duration_unit, duration_range_id)
      SELECT * FROM UNNEST(
        ${jobIds}::uuid[],
        ${indices}::integer[],
        ${values}::integer[],
...
export async function claimNextAutoDatasetJobItem(jobId: string, staleAfterMinutes = 10): Promise<AutoDatasetJobItem | null> {
  ...
  const rows = await sql`
    ...
    WHERE item.id = (
      SELECT candidate.id
      FROM ops_ml_dataset_build_job_items AS candidate
      WHERE candidate.job_id = ${jobId}
        AND (
          candidate.status = 'pending'
          OR (candidate.status = 'running' AND candidate.claimed_at < NOW() - (${staleAfterMinutes} * INTERVAL '1 minute'))
        )
  ...
  return {
    ...
    rangeId: String(rows[0].duration_range_id),
...
export async function completeAutoDatasetJobItem(jobId: string, itemId: number): Promise<void> {
  ...
  await sql`
    ...
    WHERE id = ${itemId} AND job_id = ${jobId}
  `;
...
export async function failAutoDatasetJobItem(jobId: string, itemId: number, error: string): Promise<void> {
  ...
  await sql`
    UPDATE ops_ml_dataset_build_job_items
    SET status = 'failed', completed_at = NOW(), error_message = ${error.slice(0, 2000)}
    WHERE id = ${itemId} AND job_id = ${jobId}
  `;
  await refreshAutoDatasetJobStatus(jobId);
}
      ...
      SET completed_count = counts.completed_count,
          ...
          failures = COALESCE((
            SELECT jsonb_agg(jsonb_build_object(
              ...
              'error', COALESCE(item.error_message, 'Dataset build failed.')
            ) ORDER BY item.item_index)
            FROM ops_ml_dataset_build_job_items AS item
            WHERE item.job_id = job.id AND item.status = 'failed'
          ), '[]'::jsonb),
          status = CASE
            WHEN counts.total_count > 0 AND counts.completed_count + counts.failed_count >= counts.total_count
              THEN CASE WHEN counts.completed_count = 0 THEN 'failed' ELSE 'completed' END
            ELSE 'running'
          END,
          finished_at = CASE
            WHEN counts.total_count > 0 AND counts.completed_count + counts.failed_count >= counts.total_count
...
export async function recoverAutoDatasetJob(jobId: string): Promise<AutoDatasetJob | null> {
  const job = await getAutoDatasetJob(jobId);
  if (!job || job.status !== 'running') return job;
  return refreshAutoDatasetJobStatus(jobId);
}
