...
type ContractCapability = { type: string; expiryType: string };
...
class DerivDiscoverySession {
  ...
  async proposal(symbol: string, contractType: string, value: number, unit: DerivDurationUnit): Promise<boolean> {
...
async function discoverUnit(session: DerivDiscoverySession, symbol: string, contractType: string, unit: DerivDurationUnit): Promise<{ min: number; max: number } | null> {
  const max = MAX_PROBE[unit];
  if (!(await session.proposal(symbol, contractType, 1, unit))) return null;

  let min = 1;
  let validMax = 1;
  let invalidUpper: number | null = null;
  while (validMax < max) {
    const next = Math.min(max, validMax * 2);
    if (next === validMax) break;
    if (await session.proposal(symbol, contractType, next, unit)) validMax = next;
  ...
  if (invalidUpper !== null) {
    ...
    while (left + 1 < right) {
      ...
      if (await session.proposal(symbol, contractType, mid, unit)) left = mid;
      else right = mid;
    }
    validMax = left;
  }
...
async function discover(symbol: string): Promise<DerivDurationDiscovery> {
  ...
  try {
    ...
    for (const unit of unitsForExpiryType(preferred.expiryType)) {
      const range = await discoverUnit(session, symbol, preferred.type, unit);
    ...
    if (!ranges.length) throw new Error(`Deriv proposal capability probing found no supported Rise/Fall durations for ${symbol}.`);
