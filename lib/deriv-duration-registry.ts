...
export type DerivDurationRange = { id: string; unit: DerivDurationUnit; min: number; max: number; tradeTypes: string[]; source: 'deriv-trading-durations' };
export type DerivDurationDiscovery = { symbol: string; ranges: DerivDurationRange[]; fetchedAt: string; source: 'deriv-trading-durations' };
...
function parseBoundary(record: UnknownRecord, names: string[], fallbackUnit: DerivDurationUnit | null) {
...
async function requestTradingDurations(symbol: string, timeoutMs = 10_000): Promise<DerivDurationDiscovery> {
  ...
  return new Promise<DerivDurationDiscovery>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => { if (!handled) { handled = true; try { ws.close(); } catch {} reject(new Error(`Deriv duration discovery timed out for ${symbol}.`)); } }, timeoutMs);
    const fail = (error: Error) => { if (handled) return; handled = true; clearTimeout(timeout); try { ws.close(); } catch {} reject(error); };

    ws.send(JSON.stringify({ trading_durations: 1, req_id: reqId }));
    ws.on('message', (data: WebSocket.Data) => {
      try {
        ...
        if (!ranges.length) { fail(new Error(`Deriv returned no supported trading-duration ranges for ${symbol}.`)); return; }
        handled = true; clearTimeout(timeout); try { ws.close(); } catch {}
        resolve({ symbol, ranges, fetchedAt: new Date().toISOString(), source: 'deriv-trading-durations' });
      } catch (error) { fail(error instanceof Error ? error : new Error('Invalid Deriv duration response.')); }
    });
    ws.on('error', (error) => fail(error instanceof Error ? error : new Error('Deriv duration WebSocket error.')));
    ws.on('close', () => { if (!handled) fail(new Error(`Deriv duration discovery connection closed before data was received for ${symbol}.`)); });
  });
}
