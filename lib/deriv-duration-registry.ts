...
export type DerivDurationRange = {
  ...
  max: number;
  tradeTypes: string[];
  source: 'deriv-trading-durations' | 'deriv-contracts-for';
};
...
function parseDurationToken(
  ...
  if (typeof value === 'number') {
    return Number.isSafeInteger(value) && value > 0 && fallbackUnit ? { value, unit: fallbackUnit } : null;
  }
  ...
  if (fallbackUnit && /^\d+$/.test(text)) {
    ...
    return Number.isSafeInteger(parsed) && parsed > 0 ? { value: parsed, unit: fallbackUnit } : null;
  }
...
/**
 * The legacy/public trading_durations response is hierarchical. This walker
 * deliberately understands the documented symbol -> trade_durations ->
 * durations shape while remaining tolerant of wrapper objects such as market,
 * submarket and data nodes.
 */
function collectTradeDurationNodes(
  value: unknown,
  symbol: string,
  inheritedSymbol = '',
): Array<{ durations: unknown[]; tradeType: string }> {
  if (Array.isArray(value)) {
    return value.flatMap((item) => collectTradeDurationNodes(item, symbol, inheritedSymbol));
  }
  ...
  const directSymbol =
    typeof record.symbol === 'string'
      ? record.symbol
      : typeof record.underlying_symbol === 'string'
...
function parseTradingDurations(response: UnknownRecord, symbol: string): DerivDurationRange[] {
  ...
  for (const node of collectTradeDurationNodes(response.trading_durations, symbol)) {
    for (const rawDuration of node.durations) {
      ...
      ranges.push({
        id: `${symbol}:${node.tradeType || 'unknown'}:${min.unit}:${min.value}-${max.value}`,
        unit: min.unit,
        min: min.value,
        max: max.value,
...
function parseContractsFor(response: UnknownRecord, symbol: string): DerivDurationRange[] {
  ...
  for (const rawItem of available) {
    ...
    ranges.push({
      id: `${symbol}:${String(item.contract_type ?? 'unknown')}:${min.unit}:${min.value}-${max.value}`,
      unit: min.unit,
      min: min.value,
      max: max.value,
      tradeTypes: item.contract_type ? [String(item.contract_type)] : [],
...
async function requestDeriv(
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.on('close', () => {
      if (!handled) fail(new Error(`Deriv ${expectedMsgType} connection closed before data was received for ${symbol}.`));
    });
  });
}
...
export async function getDerivDurationDiscovery(symbol: string): Promise<DerivDurationDiscovery> {
  ...
  try {
    ...
    // The public API may reject legacy discovery on a particular environment.
    // Fall through to contracts_for rather than inventing a duration range.
  }
...
export function durationToSeconds(value: number, unit: DerivDurationUnit): number | null {
  return Number.isSafeInteger(value) && value > 0 && unit !== 't' ? value * UNIT_SECONDS[unit] : null;
}
...
/**
 * Expands broker-advertised ranges into a bounded deterministic training grid.
 * Small ranges are exhaustive; very large continuous ranges use their explicit
 * boundaries to avoid accidentally requesting tens of thousands of datasets.
 */
