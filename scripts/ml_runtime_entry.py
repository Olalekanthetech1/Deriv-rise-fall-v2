...
def backtest(ticks, symbol, horizons, asset, min_confidence=None, stake=None, payout_rate=None):
    ...
    prices = [runtime.f(t.get('price')) for t in ticks]
    required_ticks = runtime.SEQ + max(int(h) for h in horizons)
    if len(prices) <= required_ticks:
        raise ValueError(f'Backtest requires more than {required_ticks} ticks')
    ...
    for raw_h in horizons:
        ...
        for i in range(start, end):
            ...
            probability_up = float(pred[1]); probability_down = float(pred[0])
        ...
        matrix[str(h)] = {
            'horizonSecs': h, 'available': True, 'trades': trades, 'wins': wins, 'losses': losses, 'rejected': rejected,
...
def dispatch(r):
    contract = r.get('schemaContract')
    runtime.configure_schema(contract)
    action = r.get('action')
    if action == 'predict':
    ...
    if action == 'list_models':
        return {'success': True, 'id': r.get('id'), 'schemaFingerprint': runtime.require_schema()['schemaFingerprint'], 'models': [p.name for p in runtime.MODEL_DIR.glob('*.pkl')]}
