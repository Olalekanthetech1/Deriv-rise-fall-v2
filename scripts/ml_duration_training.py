...
Node owns dataset construction, feature definitions, normalization and splits.
Python receives only canonical vectors/sequences and trains the native model.
"""
...
def _partition(payload: dict[str, Any], key: str, sequence: bool = False):
    ...
    vectors_key = "featureSequences" if sequence else "featureVectors"
    vectors, labels = data.get(vectors_key), data.get("labels")
    ...
    if sequence: X = np.asarray([[[float(x) for x in row] for row in seq] for seq in vectors], dtype=np.float32)
    else: X = np.asarray([native.validate_vector(vec) for vec in vectors], dtype=np.float32)
...
def train_partitioned(kind: str, payload: dict[str, Any]) -> dict[str, Any]:
    kind = native.validate_model_type(kind); schema = native.require_schema(); symbol = str(payload.get("symbol") or "").strip()
    duration_key = int(payload.get("effectiveHorizonTicks")); duration_value = payload.get("durationValue"); duration_unit = str(payload.get("durationUnit") or ""); duration_seconds = payload.get("durationSeconds")
    ...
    train_data = _partition(payload, "trainSequenceDataset", True) if family == "sequential" else _partition(payload, "trainTabularDataset")
    validation_data = _partition(payload, "validationSequenceDataset", True) if family == "sequential" else _partition(payload, "validationTabularDataset")
    Xt, yt = train_data; Xv, yv = validation_data; _require_two_classes(yt, "TRAINING"); _require_two_classes(yv, "VALIDATION")
    metrics: dict[str, float] = {}; engine: str
    ...
    if family == "sequential":
        ...
        model = train_deep(kind, Xt, yt, epochs=int(hyper.get("epochs",8)), batch_size=int(hyper.get("batchSize",64)), lr=float(hyper.get("learningRate",0.001)))
        import torch
        with torch.no_grad():
            state = {key: value.cpu() for key, value in model.state_dict().items()}; probabilities = predict_deep(kind, state, Xv)
        predictions = np.argmax(probabilities, axis=1)
        metrics = {"accuracy": round(float(accuracy_score(yv,predictions))*100.0,3), "logLoss": round(float(log_loss(yv,probabilities,labels=[0,1])),6)}
        model_record = {"modelType":kind,"state_dict":state,"validation":metrics,"trainedAt":time.time()}; engine = f"Trained native PyTorch {kind} from persisted duration partition"
    else:
        if kind == "xgboost":
        ...
        if kind in {"xgboost","lightgbm","catboost"}:
            probabilities=model.predict_proba(Xv); predictions=np.argmax(probabilities,axis=1); metrics={"accuracy":round(float(accuracy_score(yv,predictions))*100.0,3),"logLoss":round(float(log_loss(yv,probabilities,labels=[0,1])),6)}
        model_record={"modelType":kind,"model":model,"validation":metrics,"trainedAt":time.time()}

    artifact = save_duration(kind,symbol,duration_value,duration_unit,model_record)
    model_id=f"{symbol}_{duration_unit}{duration_value}_{kind}"
