...
function StatusBadge({ status }: { status: AdminSection['status'] }) {
  const label = status === 'complete' ? 'COMPLETE' : status === 'current' ? 'CURRENT' : status === 'next' ? 'NEXT' : 'PLANNED';
  return <span className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold tracking-wider ${status === 'complete' ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-300' : status === 'current' ? 'border-amber-400/20 bg-amber-400/10 text-amber-200' : status === 'next' ? 'border-cyan-400/20 bg-cyan-400/10 text-cyan-200' : 'border-white/10 bg-white/5 text-slate-300'}`}>{label}</span>;
}
