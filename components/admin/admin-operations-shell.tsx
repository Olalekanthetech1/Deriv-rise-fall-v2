...
import { Activity, ArrowRight, BarChart3, Beaker, BrainCircuit, CheckCircle2, Database, FlaskConical, Gauge, KeyRound, Lock, LogOut, Network, Radio, RefreshCw, Server, ShieldCheck, TerminalSquare } from 'lucide-react';

type AdminSection = { id: string; title: string; description: string; status: 'next' | 'planned' | 'complete'; icon: ComponentType<{ className?: string }>; };

const sections: AdminSection[] = [
  { id: 'command-center', title: 'Command Center', description: 'System health, production state, critical alerts and operational readiness.', status: 'complete', icon: Gauge },
  { id: 'intelligence', title: 'Trading Intelligence', description: 'Signals, confidence, strategy outcomes, features, horizons and model intelligence.', status: 'complete', icon: BrainCircuit },
  { id: 'models', title: 'Model Operations', description: 'Registry, validation, promotion, rollback and retraining lifecycle.', status: 'complete', icon: BarChart3 },
  { id: 'experiments', title: 'Testing & Research', description: 'Backtests, paper/shadow tests, multi-horizon evaluation and persistent experiment history.', status: 'complete', icon: FlaskConical },
  { id: 'infrastructure', title: 'Runtime & Infrastructure', description: 'Latency, APIs, WebSockets, database, cron and runtime diagnostics.', status: 'complete', icon: Server },
  { id: 'observability', title: 'Observability', description: 'Application, trading, ML, API, error and audit telemetry.', status: 'complete', icon: Activity },
  { id: 'security', title: 'Security & Configuration', description: 'Secrets, environment configuration, sessions and security events.', status: 'next', icon: KeyRound },
...
export default function AdminOperationsShell() {
  ...
  const handleLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setAuthError(null);
    try {
      const response = await fetch('/api/admin/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key: passkey }) });
      const data = await response.json();
      if (!response.ok || !data?.success) { setAuthError(data?.error || 'Authentication failed.'); return; }
      setPasskey('');
  ...
  const handleLogout = async () => {
    try { await fetch('/api/admin/auth', { method: 'DELETE' }); } finally { setIsAuthenticated(false); }
  };
