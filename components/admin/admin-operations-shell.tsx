...
type AdminSection = { id: string; title: string; description: string; status: 'next' | 'planned'; icon: ComponentType<{ className?: string }>; };
...
function StatusBadge({ status }: { status: AdminSection['status'] }) {
  return <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[10px] font-semibold tracking-wider text-slate-300">{status === 'next' ? 'NEXT' : 'PLANNED'}</span>;
}
