  ...
  return (
    <>
      <div className="fixed bottom-3 left-1/2 -translate-x-1/2 z-40 w-[95%] max-w-lg"><div className="rounded-2xl bg-[#0d121d]/90 border border-white/10 p-1.5 shadow-2xl backdrop-blur-xl flex items-center justify-between">
        ...
        <button type="button" onClick={() => setIsSocialOpen(true)} className="flex-1 flex flex-col items-center justify-center py-1.5 px-1 rounded-xl text-gray-400 hover:text-gray-200 transition-all"><Users className="w-4 h-4 mb-0.5" /><span className="text-[10px] font-medium tracking-tight">Social Trading</span></button>
      ...
      <SignalsDrawer isOpen={isSignalsOpen} onClose={() => setIsSignalsOpen(false)} activeSymbol={activeSymbol} signals={signals} consensus={consensus} modeRecommendations={modeRecommendations} soundEnabled={soundEnabled} onToggleSound={onToggleSound} winStats={winStats} onAutoFillTrade={onAutoFillTrade} onQuickExecute={onQuickExecute} isBuying={isBuying} />
